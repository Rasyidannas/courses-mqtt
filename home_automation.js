...
// Ensure connect is called after DOM is loaded and APP is defined
document.addEventListener("DOMContentLoaded", function() {
    APP.HomeAutomation.Manager.connect();

    // Add event listeners for color pickers
    for (let i = 1; i <= 3; i++) {
        const picker = document.getElementById(`color-picker-led-${i}`);
        if (picker) {
            picker.addEventListener('change', function() {
                APP.HomeAutomation.Manager.updateLed(i, this.jscolor);
            });
        }
    }
});
